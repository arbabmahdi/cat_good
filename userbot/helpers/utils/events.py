    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.from_id is None:
                if not noedits:
                    await edit_delete(catevent, "`خب این یک مدیر ناشناس است!`")
                return None, None
        ...
        if not args:
            if not noedits:
                await edit_delete(
                    catevent, "`نام کاربری، شناسه یا پاسخ کاربر را ارسال کنید!`", 5
                )
            return None, None
    except Exception as e:
        LOGS.error(str(e))
    if not noedits:
        await edit_delete(catevent, "__خطا: برای این کار باید گزینه یوزر بن شما در گروه فعال باشد__")
    return None, None
